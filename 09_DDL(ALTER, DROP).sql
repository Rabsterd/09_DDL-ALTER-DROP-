...
ALTER TABLE DEPT_COPY 
MODIFY DEPT_TITLE NULL; -- DEPT_TITLE 컬럼을 NULL로 수정
...
-- DEPARTMENT 테이블 복사 DEPT_COPY 생성
CREATE TABLE DEPT_COPY
AS SELECT * FROM DEPARTMENT;
...
ALTER TABLE DEPT_COPY
ADD CONSTRAINT D_COPY_PY PRIMARY KEY(DEPT_ID);
...
--> 해결 방법
-- 1) 자식, 부모 테이블 순서로 삭제
-- 2) ALTER를 이용해서 FK 제약조건을 삭제 후 TB1 삭제
...
--> DDL 수행 중에 DML 수행을 하면 DML이 COMMIT을 수행하지 않아도 자동 COMMIT됨
